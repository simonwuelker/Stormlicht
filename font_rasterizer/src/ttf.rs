...
pub fn parse_font_face(bytes: &[u8]) -> Result<(), TTFParseError> {
    ...
    let loca_table = offset_table
        .get_table(LOCA_TAG)
        .ok_or(TTFParseError::MissingTable)?;
    let a_offset = loca::get_glyph_offset(
        &bytes[loca_table.offset()..],
        622,
    ...
    let glyf_entry = offset_table
        ...
        .unwrap();
    let glyph_table =
        glyf::GlyphOutlineTable::new(&bytes, glyf_entry.offset(), glyf_entry.length());
    let a_glyph = glyph_table.get_glyph_outline(a_offset);

    println!("{:?}", a_glyph);
    println!("A has {} points", a_glyph.num_points());
    for (i, p) in a_glyph.points().enumerate() {
        println!("{i} => {:?}", p);
    }
