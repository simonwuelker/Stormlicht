...
impl<'a> LocaTable<'a> {
    ...
    pub fn get_glyph_offset(&self, glyph_index: GlyphID) -> u32 {
        if self.is_short {
            // Short table, u16
            // Indexing is done in words
            // Also, the offset / 2 is stored (don't ask me why)
            read_u16_at(self.data, glyph_index.numeric() as usize * 2) as u32 * 2
        } else {
            // Long table, u32
            // Indexing is done in bytes
            read_u32_at(self.data, glyph_index.numeric() as usize)
        }
    }
}
