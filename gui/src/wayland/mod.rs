...
impl WaylandHeader {
    ...
    pub fn as_bytes(self) -> [u8; 8] {
        ...
        buffer[4..8].copy_from_slice(
            &((self.message_size as u32) << 16 | self.opcode as u32).to_ne_bytes(),
        );
        buffer
    }
    ...
    pub fn read<R: Read>(mut reader: R) -> Result<Self, WaylandError> {
        ...
        reader
            ...
            .map_err(|_| WaylandError::FailedToReadFromSocket)?;
...
pub fn try_init() -> Result<Option<()>, WaylandError> {
    ...
    let fd = match env::var("WAYLAND_SOCKET") {
        ...
        _ => {
            match env::var("XDG_RUNTIME_DIR") {
                Ok(runtime_dir) => Path::new(&runtime_dir).join(
                    &env::var("WAYLAND_DISPLAY")
                        .map_or(PathBuf::from("wayland-0"), |dir| PathBuf::from(dir)),
                ),
                _ => {
                    // can't init wayland
                    return Ok(None);
    ...
    // Bind wayland socket
    let mut wayland_stream =
        UnixStream::connect(&fd).map_err(|_| WaylandError::FailedToBindSocket(fd))?;

    // Bind registry interface
    let header = WaylandHeader::new(1, 12, 1);
    let body: [u8; 4] = wl_registry::WL_REGISTRY_ID.0.to_ne_bytes();
    wayland_stream
        .write_all(header.as_bytes().as_slice())
        .map_err(|_| WaylandError::FailedToWriteToSocket)?;
    wayland_stream
    ...
    wayland_stream
        ...
        .map_err(|_| WaylandError::FailedToCloseConnection)?;
    Ok(Some(()))
}
