...
fn derive_argumentparser(input: TokenStream) -> Result<TokenStream, MacroError> {
    ...
    let optional_arguments_found = std::iter::repeat("None")
        .take(num_args_passed_as_options)
        .collect::<Vec<&'static str>>()
        .join(",");

    let positional_arguments_found = std::iter::repeat("None")
        .take(num_args_passed_by_position)
        .collect::<Vec<&'static str>>()
        .join(",");
