...
fn read_helper_attribute<I: Iterator<Item = TokenTree>>(
    ...
    while helper_arg_tokens.peek().is_some() {
        ...
        if helper_key_requires_value(&key)? {
            ...
            let value = ident_or_error!(helper_arg_tokens)?;
        ...
        .map(|(index, arg)| {
            format!(
                "{}: match &positional_arguments[{index}] {{ 
                        ...
                        None => if {} {{ Default::default() }} else {{ return Err(::cli::CommandLineParseError::MissingRequiredArgument(\"{}\")) }},
                }}", arg.destination.name, arg.is_optional, arg.destination.name
            )
        })
        .collect::<Vec<String>>()
        .join(",");
            ...
            format!(
                "{}: match &options[{index}] {{ 
                        Some(val_or_none) => match val_or_none {{ 
                            Some(val) => Some(val.parse().map_err(|_| ::cli::CommandLineParseError::InvalidArguments)?),
                ...
                }}", arg.destination.name, arg.is_optional, handle_param_passed_as_flag = if arg.is_flag { "true" } else { "return Err(::cli::CommandLineParseError::NotAFlag(\"{0}\"));" }
            )
        })
        .collect::<Vec<String>>()
        .join(",");
    ...
    let autogenerated_code = format!("
        ...
        impl ::cli::CommandLineArgumentParser for {struct_ident} {{
            fn parse() -> Result<Self, ::cli::CommandLineParseError> {{
                ...
                let mut options: [Option<Option<String>>; {num_args_passed_as_options}] = [None; {num_args_passed_as_options}];
                let mut positional_arguments: [Option<String>;  {num_args_passed_by_position}] = [None; {num_args_passed_by_position}];
